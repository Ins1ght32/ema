...
export default function AppView() {
  ...
  const fetchTimelineDataQuery = async () => {
    ...
    const response = await fetch(`http://localhost:6969/timeline-data-query?categoryID=${categoryID}&vendorID=${vendorID}&numProducts=${numProducts}`);
  ...
  useEffect(() => {
    ...
    const fetchPieDataQuery = async () => {
      const categoryID = sessionStorage.getItem("categoryID");
      const vendorID = sessionStorage.getItem("vendorID"); 
      const response = await fetch(`http://localhost:6969/pie-data-query?categoryID=${categoryID}&vendorID=${vendorID}`);
      const json = await response.json();
    ...
    const testPost = async () => {
      const response = await fetch('http://localhost:6969/test-post', {
        method: "POST",
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ message: test_post_msg }),
      });
